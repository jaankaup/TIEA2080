...
#            session['joukkue']['salasana'] = d.hexdigest().decode("UTF-8")
        ...
        newJoukkue = {'id':
                session['joukkue']['id'],'nimi':session['tempJNimi'],
                'sarja':newSarja['id'],'jasenet':unicode(json.dumps(js)),
                'salasana': u""}
...
def checkPasswordVT3(pwd):
    token = hashlib.sha512()
    token.update("erkkiajaamopolla")
    token.update(pwd)
    digest = "\x04\xaa\x19\x1b\x07\xd8]\xbe\xf6\x08UF\x1c@\xf9\xc1\x9f\n\xb2\xc0\xcf\x1b\xad\x81R}\r\x91\x9eFNhj\xab\x80\xddKD\x90\xbe0\x0co\xb6U0\xb6O\xadd\x8e\xb6$\xc5\xc8}\xf8\xa9\x96\x0c\xa2AJ\x86"
    return token.digest() == digest

#########################################################################

def checkAdminPasswordVT3(pwd):
    token = hashlib.sha512()
    token.update("erkkiajaamopolla")
    token.update(pwd)
